...
def make_calls():
     ...
     calls = Call.objects.filter(complete=False, date__gte=now-interval, date__lt=now).order_by('priority')
     for call in calls:
         ...
         if call.priority == 1:
             ...
             past_p1_cnt = Call.objects.filter(subject=call.subject, survey=call.survey, priority=1, date__lt=now-interval).count()
             past_complete_cnt = Call.objects.filter(subject=call.subject, survey=call.survey, date__lt=now-interval, complete=True).count()
