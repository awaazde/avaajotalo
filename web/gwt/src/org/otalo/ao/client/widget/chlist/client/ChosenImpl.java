 ...
 * the License.
 */
package org.otalo.ao.client.widget.chlist.client;

import static com.google.gwt.query.client.GQuery.$;
import static com.google.gwt.query.client.GQuery.document;

import org.otalo.ao.client.widget.chlist.client.SelectParser.GroupItem;
import org.otalo.ao.client.widget.chlist.client.SelectParser.OptionItem;
import org.otalo.ao.client.widget.chlist.client.SelectParser.SelectItem;
import org.otalo.ao.client.widget.chlist.event.ChosenChangeEvent;
import org.otalo.ao.client.widget.chlist.event.ChosenEvent;
...
import org.otalo.ao.client.widget.chlist.resources.ChozenCss;
import org.otalo.ao.client.widget.chlist.resources.Resources;
...
public class ChosenImpl {
    ...
    private void bind() {
        ...
        if (isMultiple) {
            ...
            searchField.mouseup(new Function() {
                @Override
                public boolean f(Event e) {
                	resultsHide();
                	return false;
            ...
            searchField.mousedown(new Function() {
                @Override
                public boolean f(Event e) {
                	resultsHide();
                	return false;
    ...
    private void addNewItem(String item, String value) {
    	//if maxlimit exceeds then do not add it...
        ...
        choices++;
        //creating choice id
        String choiceId = containerId + "_c_" + choices;	
        
        //HTML template being used in creating item
       /* @Template("<li class=\"{1}\" id=\"{0}\"><span>{2}</span><a href=\"javascript:void(0)\" class=\"{3}\" " +
                "rel=\"{4}\"></a></li>")
        SafeHtml choice(String id, String searchChoiceClass, String content,
                String searchChoiceCloseClass, String rel);*/
        //adding new selected item in searchcontainer
        searchContainer.before(ChozenTemplate.templates.choice(choiceId, css.searchChoice(),
                item, css.searchChoiceClose(), "" + choices).asString());
        //binding event
       ...
       //adding item into select
       insertItemInSelect(item, value, selectElement.getLength());
    ...
    private boolean isAlreadyExist(String itemValue) {
    	if(selectElement == null || selectElement.getLength() == 0)
    		return false;
    	
    	//iterating through the list of options
    	for(int index=0;index<selectElement.getLength();index++) {
    		if(itemValue.equalsIgnoreCase(selectElement.getOptions().getItem(index).getValue()))
    			return true;
    	}
